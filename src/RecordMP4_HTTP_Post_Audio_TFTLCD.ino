...
char ssid[] = "jiun";            // your network SSID (Home WiFi or Smartphone Hotspot)
char pass[] = "12345678";        // your network password
int status = WL_IDLE_STATUS;

//char server[] = "192.168.0.1";   // the server IP running HTTP server on PC
//char server[] = "36.225.67.36";
...
unsigned long buttonPressTime = 0;  
      // variable to store the time when button was pressed
...
void encodeMP4andsendHttpPostRequest()
{
    ...
    if (!fs.open(filepath)) {
    Serial.print("[ERROR] 文件不存在: ");
    Serial.println(filepath);
    
  }
  else{
  Serial.print("[INFO] 文件存在: ");
  Serial.println(filepath);
  }
